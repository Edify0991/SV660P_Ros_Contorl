...
volatile float kp[4] = {5, 5, 5, 5};
volatile float kd[4] = {5, 5, 5, 5};
volatile float pos[4] = {0, 0, 0, 0};
volatile float spd[4] = {20, 20, 20, 20};
...
void AnkleMotor_Send_Fcn(int devs, int channel)
{
    ...
    {
        // 使用send_motor_ctrl_cmd，传入参数为弧度制
        float pos_rad = pos[i] / 360.0f * 2 * PI;
        float spd_rad = spd[i] / 360.0f * 2 * PI;
        // 不同的指令有不同的倍率
        float spd_x10 = spd[i] * 10.0f;
        switch(state[i])
        {
            case ZEROS_SETTING_STEP:
            ...
            break;
            case POS_CONTROL_STEP:
                ankle_motor_ports[i]->set_motor_position(pos[i], spd_x10, limit_current[i], MIX_CONTROL_ACK);
            break;
            case SPEED_CONTROL_STEP:
                ankle_motor_ports[i]->set_motor_speed(spd[i], limit_current[i], MIX_CONTROL_ACK);
            break;
            default:
            break;
        }
...
void AnkleMotor_Rcv_Fcn(int devs, int channel)
{
    ...
    while(1)
    {
        if(reclen = CAN_Receive(devs, channel, rcv_msg, 300, 100)>0)
        {
            ...
            std::cout<<"pos:"<<i<<":"<<ankle_motor_ret_msg.pos / PI * 180<<std::endl;
