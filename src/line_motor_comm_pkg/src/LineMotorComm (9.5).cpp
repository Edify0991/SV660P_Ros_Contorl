...
// 踝关节电机控制命令通信包 
line_motor_comm_pkg::ankleMotorMsgCmd ankle_motor_ld_msg, ankle_motor_lu_msg;
line_motor_comm_pkg::ankleMotorMsgCmd ankle_motor_rd_msg, ankle_motor_ru_msg;
// 踝关节电机指令输入函数
...
void AnkleMotorPubCallback(const line_motor_comm_pkg::ankleMotorMsgBack::ConstPtr& msg, int anklemotorID)
{
	switch(anklemotorID)
	{
		...
		case 2:
		{
			...
			// std::cout<<"当前右腿左侧电机位置值为："<<current_pos[2]<<std::endl;
			break;
		}
		case 3:
		{
			...
			// std::cout<<"当前右腿右侧电机位置值为："<<current_pos[3]<<std::endl;
			break;
		}
	}
}
...
{	
	...
	// 创建踝关节电机消息订阅和发布
	ros::Publisher ankle_motor_ld_cmd_pub = nh.advertise<line_motor_comm_pkg::ankleMotorMsgCmd>(ankle_motor_name[0]+"_cmd", 1);
	ros::Publisher ankle_motor_lu_cmd_pub = nh.advertise<line_motor_comm_pkg::ankleMotorMsgCmd>(ankle_motor_name[1]+"_cmd", 1);
	ros::Publisher ankle_motor_rd_cmd_pub = nh.advertise<line_motor_comm_pkg::ankleMotorMsgCmd>(ankle_motor_name[2]+"_cmd", 1);
	ros::Publisher ankle_motor_ru_cmd_pub = nh.advertise<line_motor_comm_pkg::ankleMotorMsgCmd>(ankle_motor_name[3]+"_cmd", 1);
	
	int anklemotorID = 0;
	ros::Subscriber ankle_motor_ld_state_sub = nh.subscribe<line_motor_comm_pkg::ankleMotorMsgBack>(ankle_motor_name[0]+"_state", 1, boost::bind(&AnkleMotorPubCallback, _1, anklemotorID));
	anklemotorID = 1;
	ros::Subscriber ankle_motor_lu_state_sub = nh.subscribe<line_motor_comm_pkg::ankleMotorMsgBack>(ankle_motor_name[1]+"_state", 1, boost::bind(&AnkleMotorPubCallback, _1, anklemotorID));
	anklemotorID = 2;
	ros::Subscriber ankle_motor_rd_state_sub = nh.subscribe<line_motor_comm_pkg::ankleMotorMsgBack>(ankle_motor_name[2]+"_state", 1, boost::bind(&AnkleMotorPubCallback, _1, anklemotorID));
	anklemotorID = 3;
	ros::Subscriber ankle_motor_ru_state_sub = nh.subscribe<line_motor_comm_pkg::ankleMotorMsgBack>(ankle_motor_name[3]+"_state", 1, boost::bind(&AnkleMotorPubCallback, _1, anklemotorID));
	...
	// {
		...
		// ankle_motor_rd_msg.pos = -40;
		// ankle_motor_rd_msg.spd = 20;
		// ankle_motor_rd_msg.limit_current = 5;
		// ankle_motor_ru_msg.motor_state = POS_CONTROL_STEP;
		// ankle_motor_ru_msg.pos = 0;
		// ankle_motor_ru_msg.spd = 20;
		// ankle_motor_ru_msg.limit_current = 5;
		...
		// ankle_motor_ld_msg.pos = -30;
		// ankle_motor_ld_msg.spd = 20;
		// ankle_motor_ld_msg.limit_current = 5;
		// ankle_motor_lu_msg.motor_state = POS_CONTROL_STEP;
		// ankle_motor_lu_msg.pos = -20;
		// ankle_motor_lu_msg.spd = 20;
		// ankle_motor_lu_msg.limit_current = 5;
		// ankle_motor_lu_cmd_pub.publish(ankle_motor_lu_msg);
		// ankle_motor_ld_cmd_pub.pub<
	// {
	// 	ros::spinOnce();
		// ankle_motor_ll_msg.motor_state = POS_CONTROL_STEP;
		// ankle_motor_ll_msg.pos = 0;
		// ankle_motor_ll_msg.spd = 20;
		// ankle_motor_ll_msg.limit_current = 5;
		// ankle_motor_lr_msg.motor_state = POS_CONTROL_STEP;
		// ankle_motor_lr_msg.pos = 0;
		// ankle_motor_lr_msg.spd = 20;
		// ankle_motor_lr_msg.limit_current = 5;
	...
	//定义B1电机
	JointMotor motorL_0(0, MotorType::B1, nh, motor_name_[1]);//, test_motor_2(0, MotorType::A1);
	JointMotor motorL_2(2, MotorType::B1, nh, motor_name_[0]);
	JointMotor motorR_0(0, MotorType::B1, nh, motor_name_[3]);
	JointMotor motorR_2(2, MotorType::B1, nh, motor_name_[2]);
	...
	// while(1)
	{
		...
		// 脚踝电机回零
	// ankle_motor_ld_msg.motor_state = POS_CONTROL_STEP;
	// ankle_motor_ld_msg.pos = 0;
	// ankle_motor_ld_msg.spd = 20;
	// ankle_motor_ld_msg.limit_current = 10;
	// ankle_motor_lu_msg.motor_state = POS_CONTROL_STEP;
	// ankle_motor_lu_msg.pos = 0;
	// ankle_motor_lu_msg.spd = 20;
	// ankle_motor_lu_msg.limit_current = 10;
	// ankle_motor_ld_cmd_pub.publish(ankle_motor_ld_msg);
	// ankle_motor_lu_cmd_pub.publish(ankle_motor_lu_msg);
	// ankle_motor_rd_msg.motor_state = POS_CONTROL_STEP;
	// ankle_motor_rd_msg.pos = 0;
	// ankle_motor_rd_msg.spd = 20;
	// ankle_motor_rd_msg.limit_current = 10;
	// ankle_motor_ru_msg.motor_state = POS_CONTROL_STEP;
	// ankle_motor_ru_msg.pos = 0;
	// ankle_motor_ru_msg.spd = 20;
	// ankle_motor_ru_msg.limit_current = 10;
	// ankle_motor_rd_cmd_pub.publish(ankle_motor_rd_msg);
	// ankle_motor_ru_cmd_pub.publish(ankle_motor_ru_msg);
	...
	std::cout << "L-0" << motorL_0_ORI << std::endl;
	std::cout << "L-2" << motorL_2_ORI << std::endl;
	std::cout << "R-0" << motorR_0_ORI << std::endl;
	std::cout << "R-2" << motorR_2_ORI << std::endl;
	// }
	// 脚踝电机回零
	ankle_motor_ld_msg.motor_state = POS_CONTROL_STEP;
	ankle_motor_ld_msg.pos = 0;
	ankle_motor_ld_msg.spd = 20;
	ankle_motor_ld_msg.limit_current = 15;
	ankle_motor_lu_msg.motor_state = POS_CONTROL_STEP;
	ankle_motor_lu_msg.pos = 0;
	ankle_motor_lu_msg.spd = 20;
	ankle_motor_lu_msg.limit_current = 15;
	ankle_motor_ld_cmd_pub.publish(ankle_motor_ld_msg);
	ankle_motor_lu_cmd_pub.publish(ankle_motor_lu_msg);
	ankle_motor_rd_msg.motor_state = POS_CONTROL_STEP;
	ankle_motor_rd_msg.pos = 0;
	ankle_motor_rd_msg.spd = 20;
	ankle_motor_rd_msg.limit_current = 15;
	ankle_motor_ru_msg.motor_state = POS_CONTROL_STEP;
	ankle_motor_ru_msg.pos = 0;
	ankle_motor_ru_msg.spd = 20;
	ankle_motor_ru_msg.limit_current = 15;
	ankle_motor_rd_cmd_pub.publish(ankle_motor_rd_msg);
	ankle_motor_ru_cmd_pub.publish(ankle_motor_ru_msg);
	...
	// 脚踝电机零位重置
	// AnkleMotorCMD(1, ZEROS_SETTING_STEP, 0, 0, 0, 0, 0, 0);
	// AnkleMotorCMD(2, ZEROS_SETTING_STEP, 0, 0, 0, 0, 0, 0);
	// AnkleMotorCMD(3, ZEROS_SETTING_STEP, 0, 0, 0, 0, 0, 0);
	// AnkleMotorCMD(4, ZEROS_SETTING_STEP, 0, 0, 0, 0, 0, 0);
	// ankle_motor_ld_cmd_pub.publish(ankle_motor_ld_msg);
	// ankle_motor_lu_cmd_pub.publish(ankle_motor_lu_msg);
	// ankle_motor_rd_cmd_pub.publish(ankle_motor_rd_msg);
	// ankle_motor_ru_cmd_pub.publish(ankle_motor_ru_msg);
	...
	for (int i = 0 ; i < 600 ; i++) {
		ankle_motor_ld_msg.motor_state = POS_CONTROL_STEP;
		ankle_motor_ld_msg.pos = -anklel_begin[i];
		ankle_motor_ld_msg.spd = 20;
		ankle_motor_ld_msg.limit_current = 15;
		ankle_motor_lu_msg.motor_state = POS_CONTROL_STEP;
		ankle_motor_lu_msg.pos = anklel_begin[i];
		ankle_motor_lu_msg.spd = 20;
		ankle_motor_lu_msg.limit_current = 15;
		ankle_motor_ld_cmd_pub.publish(ankle_motor_ld_msg);
		ankle_motor_lu_cmd_pub.publish(ankle_motor_lu_msg);
		ankle_motor_rd_msg.motor_state = POS_CONTROL_STEP;
		ankle_motor_rd_msg.pos = ankler_begin[i];
		ankle_motor_rd_msg.spd = 20;
		ankle_motor_rd_msg.limit_current = 15;
		ankle_motor_ru_msg.motor_state = POS_CONTROL_STEP;
		ankle_motor_ru_msg.pos = -ankler_begin[i];
		ankle_motor_ru_msg.spd = 20;
		ankle_motor_ru_msg.limit_current = 15;
		ankle_motor_rd_cmd_pub.publish(ankle_motor_rd_msg);
		ankle_motor_ru_cmd_pub.publish(ankle_motor_ru_msg);
	...
	for(int i = 0 ; i < 0.5 * unit_time ; i++) {      //每次走0.5个unit_time
		...
		ankle_motor_ld_msg.motor_state = POS_CONTROL_STEP;
		ankle_motor_ld_msg.pos = -anklel_begin[CntPitch - 1] + k_ankle_hip_roll * hip_roll[i % (2 * unit_time)] / PI *180.0;
		ankle_motor_ld_msg.spd = 20;
		ankle_motor_ld_msg.limit_current = 5;
		ankle_motor_lu_msg.motor_state = POS_CONTROL_STEP;
		ankle_motor_lu_msg.pos = anklel_begin[CntPitch - 1] + k_ankle_hip_roll * hip_roll[i % (2 * unit_time)] / PI *180.0;
		ankle_motor_lu_msg.spd = 20;
		ankle_motor_lu_msg.limit_current = 5;
		ankle_motor_ld_cmd_pub.publish(ankle_motor_ld_msg);
		ankle_motor_lu_cmd_pub.publish(ankle_motor_lu_msg);
		ankle_motor_rd_msg.motor_state = POS_CONTROL_STEP;
		ankle_motor_rd_msg.pos = ankler_begin[CntPitch - 1] + k_ankle_hip_roll * hip_roll[i % (2 * unit_time)] / PI *180.0;
		ankle_motor_rd_msg.spd = 20;
		ankle_motor_rd_msg.limit_current = 5;
		ankle_motor_ru_msg.motor_state = POS_CONTROL_STEP;
		ankle_motor_ru_msg.pos = -ankler_begin[CntPitch - 1] + k_ankle_hip_roll * hip_roll[i % (2 * unit_time)] / PI *180.0;
		ankle_motor_ru_msg.spd = 20;
		ankle_motor_ru_msg.limit_current = 5;
		ankle_motor_rd_cmd_pub.publish(ankle_motor_rd_msg);
		ankle_motor_ru_cmd_pub.publish(ankle_motor_ru_msg);
	...
	for(int i = CntPitch ; i < number_begin ; i++) {  
		...
		ankle_motor_ld_msg.motor_state = POS_CONTROL_STEP;
		ankle_motor_ld_msg.pos = -anklel_begin[i] + k_ankle_hip_roll * hip_roll[CntRoll % (2 * unit_time)] / PI *180.0;
		ankle_motor_ld_msg.spd = 20;
		ankle_motor_ld_msg.limit_current = 5;
		ankle_motor_lu_msg.motor_state = POS_CONTROL_STEP;
		ankle_motor_lu_msg.pos = anklel_begin[i] + k_ankle_hip_roll * hip_roll[CntRoll % (2 * unit_time)] / PI *180.0;
		ankle_motor_lu_msg.spd = 20;
		ankle_motor_lu_msg.limit_current = 5;
		ankle_motor_ld_cmd_pub.publish(ankle_motor_ld_msg);
		ankle_motor_lu_cmd_pub.publish(ankle_motor_lu_msg);
		ankle_motor_rd_msg.motor_state = POS_CONTROL_STEP;
		ankle_motor_rd_msg.pos = ankler_begin[i] + k_ankle_hip_roll * hip_roll[CntRoll % (2 * unit_time)] / PI *180.0;
		ankle_motor_rd_msg.spd = 20;
		ankle_motor_rd_msg.limit_current = 5;
		ankle_motor_ru_msg.motor_state = POS_CONTROL_STEP;
		ankle_motor_ru_msg.pos = -ankler_begin[i] + k_ankle_hip_roll * hip_roll[CntRoll % (2 * unit_time)] / PI *180.0;
		ankle_motor_ru_msg.spd = 20;
		ankle_motor_ru_msg.limit_current = 5;
		ankle_motor_rd_cmd_pub.publish(ankle_motor_rd_msg);
		ankle_motor_ru_cmd_pub.publish(ankle_motor_ru_msg);
	...
	for(int i = 0.5 * unit_time ; i < unit_time ; i++) {      // 侧摆回正
		...
		ankle_motor_ld_msg.motor_state = POS_CONTROL_STEP;
		ankle_motor_ld_msg.pos = -anklel_begin[number_begin - 1] + k_ankle_hip_roll * hip_roll[i % (2 * unit_time)] / PI *180.0;
		ankle_motor_ld_msg.spd = 20;
		ankle_motor_ld_msg.limit_current = 5;
		ankle_motor_lu_msg.motor_state = POS_CONTROL_STEP;
		ankle_motor_lu_msg.pos = anklel_begin[number_begin - 1] + k_ankle_hip_roll * hip_roll[i % (2 * unit_time)] / PI *180.0;
		ankle_motor_lu_msg.spd = 20;
		ankle_motor_lu_msg.limit_current = 5;
		ankle_motor_ld_cmd_pub.publish(ankle_motor_ld_msg);
		ankle_motor_lu_cmd_pub.publish(ankle_motor_lu_msg);
		ankle_motor_rd_msg.motor_state = POS_CONTROL_STEP;
		ankle_motor_rd_msg.pos = ankler_begin[number_begin - 1] + k_ankle_hip_roll * hip_roll[i % (2 * unit_time)] / PI *180.0;
		ankle_motor_rd_msg.spd = 20;
		ankle_motor_rd_msg.limit_current = 5;
		ankle_motor_ru_msg.motor_state = POS_CONTROL_STEP;
		ankle_motor_ru_msg.pos = -ankler_begin[number_begin - 1] + k_ankle_hip_roll * hip_roll[i % (2 * unit_time)] / PI *180.0;
		ankle_motor_ru_msg.spd = 20;
		ankle_motor_ru_msg.limit_current = 5;
		ankle_motor_rd_cmd_pub.publish(ankle_motor_rd_msg);
		ankle_motor_ru_cmd_pub.publish(ankle_motor_ru_msg);
...
void AnkleMotorCMD(int motor_num, int motor_state, float kp, float kd, float pos, float spd, float tau, float limit_current)
{
	...
	{
		case 1: // 左腿左侧电机
			ankle_motor_ld_msg.motor_state = motor_state;
			ankle_motor_ld_msg.kp = kp;
			ankle_motor_ld_msg.kd = kd;
			ankle_motor_ld_msg.pos = pos;
			ankle_motor_ld_msg.spd = spd;
			ankle_motor_ld_msg.tau = tau;
			ankle_motor_ld_msg.limit_current = limit_current;
		break;
		case 2: // 左腿右侧电机
			ankle_motor_lu_msg.motor_state = motor_state;
			ankle_motor_lu_msg.kp = kp;
			ankle_motor_lu_msg.kd = kd;
			ankle_motor_lu_msg.pos = pos;
			ankle_motor_lu_msg.spd = spd;
			ankle_motor_lu_msg.tau = tau;
			ankle_motor_lu_msg.limit_current = limit_current;
		break;
		case 3: // 右腿左侧电机
			ankle_motor_rd_msg.motor_state = motor_state;
			ankle_motor_rd_msg.kp = kp;
			ankle_motor_rd_msg.kd = kd;
			ankle_motor_rd_msg.pos = pos;
			ankle_motor_rd_msg.spd = spd;
			ankle_motor_rd_msg.tau = tau;
			ankle_motor_rd_msg.limit_current = limit_current;
		break;
		case 4: // 右腿右侧电机
			ankle_motor_ru_msg.motor_state = motor_state;
			ankle_motor_ru_msg.kp = kp;
			ankle_motor_ru_msg.kd = kd;
			ankle_motor_ru_msg.pos = pos;
			ankle_motor_ru_msg.spd = spd;
			ankle_motor_ru_msg.tau = tau;
			ankle_motor_ru_msg.limit_current = limit_current;
		break;
	}
}
